...
本文档适用于 `frp-panel-offline-v1.2.12.zip`。客户可以使用包内 `install-offline.sh` 一键部署，也可以按本文步骤手工安装已经生成的 Linux amd64/arm64 二进制。只有需要修改或复现构建时才从源码重新编译。
...
```text
frp-panel-offline-v1.2.12/
├── source/                         完整 v1.2.12 源码
├── bin/
...
```sh
sha256sum frp-panel-offline-v1.2.12.zip
```
...
```sh
unzip frp-panel-offline-v1.2.12.zip
cd frp-panel-offline-v1.2.12
```
